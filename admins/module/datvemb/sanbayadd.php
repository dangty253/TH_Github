<?php
$id = getIndex("id","");
$c = getIndex("c","");
if($c=='add') echo "<script type='text/javascript'>alert('Đã Thêm');</script>";
else if ($c=='update') echo "<script type='text/javascript'>alert('Đã cập nhật');</script>";
else if ($c=='delete') echo "<script type='text/javascript'>alert('Đã xóa');</script>";

$Sb= new Sanbay();
if ($id=="") //khong co -> them moi
{
  $c='add';
	$info ="Thêm";
	$MaSanBay="";
	$TenSanBay="";
  $TenTinhThanh="";
}
else
{
  $r = $Sb->Getfetch($id);
  $c='update';
  $info ="Sửa";
  $MaSanBay=$r['MaSanBay'];
  $TenSanBay=$r['TenSanBay'];
  $TenTinhThanh=$r['TenTinhThanh'];
}

?>
<form action="xulysanbay.php?c=<?php echo $c ?>" method="post">
<fieldset>
<legend><?php echo $info;?></legend>
<table width="50%" border="1" cellspacing="3">
  <tr>
    <td width="23%">Mã sân bay</td>
    <td width="77%"><input type="text" name="MaSanBay" value="<?php echo $MaSanBay;?>"></td>
  </tr>
  
  <tr>
    <td>Tên sân bay</td>
    <td><input type="text" name="TenSanBay" value="<?php echo $TenSanBay;?>"></td>
  </tr>
    <tr>
    <td>Tên tỉnh thành</td>
    <td><input type="text" name="TenTinhThanh" value="<?php echo $TenTinhThanh;?>"></td>
  </tr>
  <tr>
    <td colspan="2">

	<input type="submit" value="<?php echo $info ?>">
 
	<?php
    
	?></td>
    </tr>
</table>
</fieldset>
</form>